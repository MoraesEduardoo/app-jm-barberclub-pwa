 ...
 * Já entra como "confirmado" (o cliente está fisicamente na barbearia, não
 * faz sentido ficar "pendente") e marcado com is_walk_in = true, pra
 * diferenciar visualmente dos agendamentos feitos com antecedência.
 *
 * "time" é opcional — se vier vazio, usa o horário atual (encaixe imediato).
 */
export async function createWalkInAppointment({
  ...
  // Push é efeito colateral: se o envio falhar (VAPID errado, barbeiro sem
  // dispositivo inscrito, FCM fora do ar), o agendamento continua salvo e
  // a tela do painel não quebra — o erro só vai pro log do servidor.
  try {
