...
2. **Salvamento** — `POST /api/push/subscribe` grava `endpoint`, `p256dh` e
   ...
   Ao abrir o Perfil, uma inscrição que já existe no navegador também é
   sincronizada novamente com o banco. (`DELETE` na mesma rota remove, ao
   desativar.)
...
curl -X POST https://SEU-DOMINIO/api/push/send \
  ...
  -H "x-push-secret: SEU_PUSH_WEBHOOK_SECRET" \
