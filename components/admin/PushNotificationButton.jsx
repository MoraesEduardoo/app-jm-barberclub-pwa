...
async function getPushServiceWorkerRegistration() {
  ...
  return Promise.race([
    ...
    new Promise((_, reject) => {
      window.setTimeout(
        () => reject(new Error('O Service Worker não ficou pronto. Atualize a página e tente novamente.')),
        10_000
      );
    }),
  ]);
...
export default function PushNotificationButton() {
  ...
  const persistSubscription = useCallback(async (subscription) => {
    if (!barber?.id) {
      ...
      try {
        ...
        if (existingSubscription) {
          ...
          if (!subscriptionsUseSameVapidKey(existingSubscription, vapidKey)) {
            if (active) {
              setErrorMessage('A chave de notificações mudou. Toque em ativar para renovar a inscrição.');
              setStatus('error');
            }
            return;
          }
          ...
          try {
            ...
            console.error('[PushNotificationButton] inscrição local não sincronizada:', error);
            if (active) {
              setErrorMessage(error.message || 'Não foi possível sincronizar a inscrição.');
              setStatus('error');
            }
          }
          return;
